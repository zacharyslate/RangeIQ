...
def test_apply_pretrained_workspace_bundle_replaces_existing_saved_boundary_files(tmp_path: Path):
    temp_settings = _temp_settings(tmp_path)
    auth_service = AuthService(temp_settings.auth_db_path)
    user = auth_service.create_user(
        email="justinwarren1984@gmail.com",
    ...
    (workspace_dir / "saved_boundary.geojson").write_text('{"type":"FeatureCollection","features":[]}', encoding="utf-8")
    (workspace_dir / "saved_boundary.kml").write_text("<kml>stale</kml>", encoding="utf-8")

    bundle_dir = tmp_path / "bundle"
    (bundle_dir / "models").mkdir(parents=True)
    (bundle_dir / "boundary").mkdir(parents=True)
    boundary_path = bundle_dir / "boundary" / "clear-creek.kml"
    boundary_path.write_text("<kml>fresh-boundary</kml>", encoding="utf-8")
    (bundle_dir / "models" / "forage_model.pkl").write_bytes(b"forage")
    (bundle_dir / "models" / "stress_model.pkl").write_bytes(b"stress")
    (bundle_dir / "models" / "metadata.json").write_text('{"selected_forage_model": "GradientBoostingRegressor"}', encoding="utf-8")
    manifest = {
    ...
    apply_pretrained_workspace_bundle(bundle_dir=bundle_dir, app_settings=temp_settings)

    assert not (workspace_dir / "saved_boundary.geojson").exists()
    assert (workspace_dir / "saved_boundary.kml").read_text(encoding="utf-8") == "<kml>fresh-boundary</kml>"
